...
class ColaPrioridadLimitada(object):
    # la clase implementa una cola de prioridad con un límite máximo de elementos
    # que se pueden almacenar en la cola
    # constructor toma una argumento opcional "limite" maximo
    def __init__(self, limite=None, *args):
        # de elementos de la cola . la cola se inicializa como una lista vacia
        self.limite = limite
        self.queue = list()

    def __getitem__(self, val):  # se usa para acceder a los elementos de la cola
        return self.queue[val]

    def __len__(self):  # devuelve el numeero de elementos de la cola
        return len(self.queue)

    def append(self, x):  # Agrega elemntos a la cola . El elemento se agrega al final de la lista
        heapq.heappush(self.queue, x)  # Usando la funcion "heappush"
        # si el limite de la se a especificado
        if self.limite and len(self.queue) > self.limite:
            # y se a alcanzado el limite
            # se elimina el elemento con la mayor
            self.queue.remove(heapq.nlargest(1, self.queue)[0])

    def pop(self):  # Elimina y devulve el elemento con la mayor prioridad del la cola usando la funcio "heappop"

        return heapq.heappop(self.queue)

    def extend(self, iterable):  # Agrega varios elementos a la cola
        for x in iterable:
            self.append(x)

    def clear(self):  # elimina todos los elementos de la cola
        for x in self:
            self.queue.remove(x)

    def remove(self, x):  # Elimina una elemento especifico de la cola
        self.queue.remove(x)

    def sorted(self):  # devuelve una lista ordenada de los elementos de la cola usando al funcion"nsmallest"
...
def _generic_arg(iterable, funcion, mejor_funcion):
    ...
    candidatos = [x for x, value in zip(
        iterable, valores) if value == mejor_valor]
...
def primero_profundidad_limitada_iterativa(problema, busqueda_en_grafo=False):
    ...
    if viewer:
        viewer.event('no_more_runs', solucion,
                     'returned after %i runs' % limite)
